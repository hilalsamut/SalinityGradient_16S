...
# Goals
1. Infer Errrors in our sequences, separately on forward and reverse reads.
...
6. Remove chimeras. 
7. Write out relevant files: `asv_table`, `asvs_fasta`, `tax_table`, and `sample_data`.
...
filtered_forward_reads <- list.files(filtered_fastqs_path, pattern = "R1_filtered.fastq.gz",
                                     full.names = TRUE)
...
# Reverse vector
filtered_reverse_reads <- list.files(filtered_fastqs_path, pattern = "R2_filtered.fastq.gz",
                                     full.names = TRUE)
...
typeof(dada_forward)
dada_forward$`022um-Control_R2_filtered.fastq.gz`
...
# Inspect further
head(merged_ASVs)
merged_ASVs$`20210602-MA-ABB1W_R1_filtered.fastq.gz`
