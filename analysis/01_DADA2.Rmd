...
```
# Ran these commands to symboliccaly link files to run dada2 on them
...
5. Re-inspect the quality of reads. (Intuition check)
6. Infer the error rate on the forward (R1) nad reverse (R2) reads.
**DADA2 building the error model.**
7. Apply th error model and infer ASVs on forward and reverse reads individually. 
...
# Re-inspect the quality of reads. (Intuition check)